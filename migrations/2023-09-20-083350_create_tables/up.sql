CREATE TABLE "lender" (
  "id" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  "org_name" varchar(255) NOT NULL,
  "email" varchar(255) NOT NULL UNIQUE,
  "password" varchar(255) NOT NULL,
  "created_at" timestamp NOT NULL DEFAULT NOW()
);

CREATE TABLE "users" (
  "id" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  "first_name" varchar(255) NOT NULL,
  "last_name" varchar(255) NOT NULL,
  "email" varchar(255) NOT NULL UNIQUE,
  "password" varchar(255) NOT NULL,
  "created_at" timestamp NOT NULL DEFAULT NOW()
);

CREATE TABLE "document" (
  "id" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  "lender_email" varchar(255) NOT NULL,
  "user_email" varchar(255) NOT NULL,
  "document_type" varchar(50) NOT NULL,
  "document_data" text NOT NULL,
  "updated_at" timestamp NOT NULL DEFAULT NOW()
);

CREATE TABLE "deal" (
  "id" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  "lender_id" integer NOT NULL,
  "user_id" integer NOT NULL,
  "document_id" integer NOT NULL,
  "status" varchar(50) NOT NULL,
  "updated_at" timestamp NOT NULL DEFAULT NOW()
);

CREATE TABLE "document_request" (
  "id" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  "lender_id" integer NOT NULL,
  "user_id" integer NOT NULL,
  "status" varchar(50) NOT NULL,
  "updated_at" timestamp NOT NULL DEFAULT NOW()
);

COMMENT ON COLUMN "document"."document_data" IS 'Content of the document';

ALTER TABLE "document" ADD FOREIGN KEY ("lender_email") REFERENCES "lender" ("email");

ALTER TABLE "document" ADD FOREIGN KEY ("user_email") REFERENCES "users" ("email");

ALTER TABLE "deal" ADD FOREIGN KEY ("lender_id") REFERENCES "lender" ("id");

ALTER TABLE "deal" ADD FOREIGN KEY ("user_id") REFERENCES "users" ("id");

ALTER TABLE "deal" ADD FOREIGN KEY ("document_id") REFERENCES "document" ("id");

ALTER TABLE "document_request" ADD FOREIGN KEY ("user_id") REFERENCES "users" ("id");

ALTER TABLE "document_request" ADD FOREIGN KEY ("lender_id") REFERENCES "lender" ("id");